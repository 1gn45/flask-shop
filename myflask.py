...
from shopbackendfunctions import (
    addordertoordersheet,
    getfilejsonfromdrive,
    getorderexelrowsandsvgs,
    getordernumber,
    getordersvgs,
    getordersvgs1,
    makesvg,
    prgetorderlist,
    setSvgsFinished,
    storeinvoiceidanddate,
)
from shopfunctions import (
    cartpricecounter,
    cartproductsinfo,
    countcartprice,
    csvcategories,
    ...
    loginfnc,
    omnivareturndict,
    productconfigpostvalues,
    productconfigproductnumber,
    productcustomize,
    productinputinfo,
    registerfnc,
    searchfnc,
    setorder,
    setunpaidorderincsvline,
...
# Google Drive file holding Swedbank payment notifications
...
webdata = {
    "phone": os.environ.get("seller_phone"),
    "email": os.environ.get("seller_mail"),
    "copyright": ["IgnasMakes", "www.ignasmakes.com"],
    "shopname": os.environ.get("shopname"),
    "contacts": shopcontacts,
}
seller = {
    "name": os.environ.get("seller_name"),
    "sellercode": os.environ.get("seler_idcode"),
    "selleremail": os.environ.get("seller_mail"),
    "indveikloskodas": os.environ.get("seller_businessidcode"),
    "sellerphone": os.environ.get("seller_phone"),
    "bankacc": os.environ.get("seller_bankacc"),
    "address": os.environ.get("seller_address"),
}

app.config.update(
    DEBUG=False,
    MAIL_SERVER="smtp.flockmail.com",
    MAIL_PORT=587,
...
secretkey = os.environ.get("jwt_secretcode")  # used with jwt.encode
...
# Endpoint hit by the phone app to record a manually collected payment (BETA)
...
def index():
    ...
    return render_template(
        "home.html", webdata=webdata, pagetitle=webdata["shopname"], suggestion=suggestion
    )
...
def rules():
    return render_template(
        "rules.html", webdata=webdata, pagetitle="Taisyklės - " + webdata["shopname"]
    )
...
def catalogue():
    return render_template(
        "catalogue.html",
        webdata=webdata,
        pagetitle="Katalogas - " + webdata["shopname"],
        listas=csvcategories(),
    )
...
def aboutus():
    return render_template(
        "aboutus.html", webdata=webdata, pagetitle="Apie mus - " + webdata["shopname"]
    )
...
def contacts():
    return render_template(
        "contacts.html",
        webdata=webdata,
        pagetitle="Kontaktai - " + webdata["shopname"],
        message="laabas",
    )
...
def login():
    if request.method == "POST":
        if loginfnc(request.form["email"], request.form["pwd"]) == "ok":
            ...
            session["user"] = jwt.encode(
                {"email": request.form["email"]}, secretkey, algorithm="HS256"
            )
            return redirect(url_for("index"))
    return render_template(
        "login.html", webdata=webdata, pagetitle="Prisijungti - " + webdata["shopname"]
    )
...
def userpage():
    return render_template(
        "userpage.html", webdata=webdata, pagetitle="Paskyra - " + webdata["shopname"]
    )
...
def liked():
    return render_template(
        "likedproducts.html",
        webdata=webdata,
        pagetitle="Įsiminti produktai - " + webdata["shopname"],
    )
        ...
        if request.method == "POST":
            ...
            if (
                registerfnc(
                    ...
                    email,
                    str(request.values.get("notifications")),
                )
                == "ok"
            ):
                session["logged_in"] = True
                session["user"] = jwt.encode({"email": email}, secretkey, algorithm="HS256")
                return redirect(url_for("index"))
...
def cart():
    ...
    return render_template(
        "cart.html",
        webdata=webdata,
        pagetitle="Krepšelis - " + webdata["shopname"],
        cartinfo=cartinfo,
        planeddelivery=simpledeliverycounter(),
    )
...
def shipping():
    ...
    return render_template(
        ...
        pagetitle="Užsakymas - " + webdata["shopname"],
        price=price,
        omnivalist=omnivareturndict(),
    )
...
def checkoutpage():
    ...
    return render_template(
        ...
        webdata=webdata,
        pubkey=stripe_keys["publishable_key"],
        pagetitle="Apmokejimas - " + webdata["shopname"],
        cartdata=cartdata,
        buyerdatadumped=json.dumps(buyerdata),
        buyerdata=buyerdata,
        deliveryprices=deliveryprices,
        cartprice=countcartprice(buyerdata["delivery type"], cartdata),
        planeddelivery=simpledeliverycounter(),
    )
...
def invoice():
    sourceHtml = render_template(
        "invoice/Invoice.html",
        seller=seller,
        cartdata={
            "orderid": "Užsakymas123456",
            "cartprice": 12345,
            "deliverytype": "omniva",
            "deliveryprice": 69,
            "cartlist": [
                {
                    "name": "Puodelio padekliukas",
                    "id": "5",
                    "price": 699,
                    "number": 3,
                    "fullprice": 210,
...
def banktransaction():
    ...
    if fullprice == int(request.form.getlist("cartprice")[0]):
        ordernumber = setunpaidorderincsvline(
            buyerdata, userdict, fullprice, "unpayed"
        )  ###############issaugomas uzsakymas
        cartlist = []
        for a in cartdata:
            singleproduct = {}
        ...
        cartdata = {
            ...
            "cartprice": int(fullprice),
            "deliverytype": buyerdata["delivery type"],
            "deliveryprice": deliveryprices["ltu"][buyerdata["delivery type"]],
            "cartlist": cartlist,
        }
        invoicehtml = render_template(
            "invoice/Invoice.html",
            seller=seller,
            cartdata=cartdata,
            buyerdata=buyerdata,
        ...
        emailhtml = render_template(
            "email templates/order.html",
            fullprice=fullprice,
            h3=h3,
            shopname=webdata["shopname"],
            seller=seller,
            ordernumber=ordernumber[0],
            deliverydate=simpledeliverycounter(),
        )
        msg = Message(
            webdata["shopname"] + " Užsakymas priimtas",
            sender="info@kaziukas24.lt",
            recipients=[buyerdata["email"]],
        )
        msg.html = emailhtml
...
def charge():
    try:
        customer = stripe.Customer.create(email="sample@customer.com", source=request.json["token"])
...
def paieska():
    try:
        ...
        return render_template(
            ...
            category=category,
            material=material,
            pricebegins=pricebegins,
            priceends=priceends,
            page=page,
            paginationlen=int(searchresults["pagination"]),
...
def preke(productid):
    ...
    return render_template(
        "productpage.html",
        webdata=webdata,
        imageid="0B5_mrYtiuzIjMjRxdXl2dEtoQlE",
        pagetitle=(str(productinfo["name"]) + " -" + webdata["shopname"]),
        productinfo=productinfo,
        suggestion=suggestion,
        productid=productid,
        modifyorcreatenew=modifyorcreatenew,
    )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if action == "showall":
                ...
                return render_template(
                    "manufacture/ProductionMain.html",
                    orderstoshow="all",
                    orderlist=orderlist,
                    swedbankMessage=swedbankMessages,
                )
...
def organizesvg():
    if request.method == "POST":
        ...
        return render_template(
            "manufacture/organizemanufacture.html",
            webdata=webdata,
            orderlist=makesvg(getorderexelrowsandsvgs(orderlist)),
        )
...
def order(orderid, openmode):
    ...
    if request.method == "POST":
        ...
        if action == "SetSvgsFinished":
            ...
            return render_template(
                "manufacture/order.html",
                orderid=orderid,
                orderinfo=orderinfo,
                ordercontent=ordersvgslistfile,
                finishedsvgslist=finishedsvgslist,
            )
    if openmode == "fast":
        ordersvgslistfile = getordersvgs1(orderinfo)
    if openmode == "slow":
...
def orderinvoice(orderid):
    ...
    for a in cartdata:
        ...
        cartlist.append(singleproduct)
    cartdata = {
        "orderid": orderinfo[0][0],
        "cartprice": int(fullprice),
        "deliverytype": orderinfo[0][13],
        "deliveryprice": deliveryprices["ltu"][orderinfo[0][13]],
        "cartlist": cartlist,
    }
    ...
    if request.method == "POST":
        ...
        buyerdata = {
            "buyer type": orderinfo[0][3],
            "name": orderinfo[0][4],
            "email": orderinfo[0][7],
            "phone": orderinfo[0][8],
            "delivery type": orderinfo[0][13],
            "omniva address": orderinfo[0][14],
            "address": orderinfo[0][15],
            "organization name": orderinfo[0][9],
            "organization id": orderinfo[0][11],
            "pvm id": orderinfo[0][12],
            "organization address": orderinfo[0][10],
        }
        invoicehtml = render_template(
            "invoice/Invoice.html",
            seller=seller,
        ...
        h3 = "Sveiki, " + buyerdata["name"] + ","
        emailhtml = render_template(
            "email templates/paymentaccepted.html",
            invoiceid=invoicetext1,
            fullprice=fullprice,
            h3=h3,
            shopname=webdata["shopname"],
            seller=seller,
            ordernumber=orderinfo[0][0],
        )
        msg = Message(
            webdata["shopname"] + " apmokėjimo patvirtinimas",
            sender="info@kaziukas24.lt",
            recipients=[buyerdata["email"]],
        )
        msg.html = emailhtml
    ...
    return render_template(
        "manufacture/orderinvoice.html",
        invoicetext=invoicetext,
        invoicetext1=invoicetext1,
        buyerdata=orderinfo[0],
        orderid=orderid,
        seller=seller,
        date=date,
        cartdata=cartdata,
...
def calibrateengravings():
    if request.method == "POST":
        ...
        if (
            workimage
            and allowed_file(workimage.filename)
            and engraveimage
            and allowed_file(engraveimage.filename)
        ):
...
def tools():
    ...
    if request.method == "POST":
        ...
        newcurrentlist = getdrivefile(currentToollistfileid)
    return render_template(
        "/cncworkshop/tools.html",
        message=message,
        currenttools=newcurrentlist,
        shopurl=shopurl,
        worntools=getdrivefile(wornouttoolsfileid),
        libraries=f360toollibrries,
...
def page_not_found(e):
    ...
    return render_template(
        "home.html", webdata=webdata, pagetitle="ShopName", suggestion=suggestion
    )
