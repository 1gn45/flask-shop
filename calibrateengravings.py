...
def calibrateimage():
    img1 = cv2.imread(
        tmpfolder + "engravetest.jpg"
    )  #######Atidarau paveiksliuka is kurio noriu paimti testo kvadratuka ir ji ivertinti
    points4, height, width = get4edgesinimage(img1)  #######randu kvadratuko 4 kampinius taskus

    pts1 = np.float32(
        [[0, 0], [1015, 0], [0, 400], [1000, 400]]
    )  ######nustatau i kokiu matmenu paveiksliuka ikelsiu testo kvadrata
    pts2 = np.float32(
        [points4[0], points4[1], points4[2], points4[3]]
    )  ######paruosiu 4tasku np float plota
    matrix = cv2.getPerspectiveTransform(
        pts2, pts1
    )  #######################gaunu perkelimo matrica pagal kuria perktreipsiu iskreipta kvadrata i mano nustatytu matmenu kvadrata
    result = cv2.warpPerspective(
        img1, matrix, (1015, 400)
    )  #######Atlieku pixeliu perkelima pagal matrica
    result = cv2.cvtColor(
        result, cv2.COLOR_BGR2GRAY
    )  ####paverciu nespalvotai kad butu paprasciau ivertint pixeliu tamsuma
    ...
    pixelsdarkness = [
        int(x * convertratio) for x in pixelsdarkness
    ]  #######convert list of pixels to values from 0 to 255
    maxvalue, minvalue = max(pixelsdarkness), min(
        pixelsdarkness
    )  #######reset new max and min values needed for further operations
