...
def cartproductsinfo(productlist):
    ...
    for n in productlist:
        returnlist.extend(
            [
                [
                    prekes[(int(n) - 1)][productinfo("prname")],
                    prekes[(int(n) - 1)][productinfo("prphoto1")],
                    prekes[(int(n) - 1)][productinfo("prprice")],
                    prekes[(int(n) - 1)][productinfo("prdiscount")],
                    prekes[(int(n) - 1)][productinfo("prid")],
                    prekes[(int(n) - 1)][productinfo("prtype")],
                ]
            ]
        )
    return returnlist
...
def suggestsimilarproducts(similarpr):
    ...
    offs = [
        [
            "prsimilarproducts",
            prekes[(int(similarpr) - 1)][productinfo("prsimilarproducts")],
            "Panašūs produktai",
        ],
        [
            "printerestsclass",
            prekes[(int(similarpr) - 1)][productinfo("printerestsclass")],
            "Siūlome taip pat peržiūrėti",
        ],
    ]
    resultdict = {}
        ...
        for id in productlist:
            if offer[1] in prekes[(int(id) - 1)][productinfo(offer[0])]:
                if id not in resultdict[offer[1]]:
                    if str(id) != str(
                        similarpr
                    ):  ###padarau kad nesiulytu tos pacios prekes kuri yra atidaryta
    ...
    for a in resultdict:
        ...
        newresultdict[a] = random.sample(
            resultdict[a], resultdictnumber
        )  #####kad nebutu klaidos ir nebandytu imti daugiau random variantu jei yra mazai is ko rinktis
    newresultdict1 = {}
...
def searchfnc(page, keywordsx, kategorijax, medziagax, nuox, ikix, interestsx, similarproductsx):
    ...
    if len(keywordsx) != 0:
        ...
        if newresultlist == []:
            ...
            newresultlist3 = []
            return {
                "pages": pagesfnc(page, newresultlist3, prekes),
                "pagination": paginationfnc(page, len(newresultlist3)),
            }
    ...
    newresultlist3 = []
    if (
        len(nuox) == 0 and len(ikix) == 0
    ):  # jei neivesta kaina grazinu paskutini paieskos rezultata baigdamas funkcija
    ...
    if (
        len(interestsx) != 0
    ):  ########patikrinam ar nereikia atrinkti rezultato pagal interesu klases
    ...
    if (
        len(similarproductsx) != 0
    ):  ########patikrinam ar nereikia atrinkti rezultato pagal interestu klases
        newresultlist4 = []
        for id in newresultlist3:
...
def pagesfnc(page, plist, prekes):
    currentpagelist1 = []
    for a in plist:
        currentpagelist1.extend(
            [
                [
                    prekes[(int(a) - 1)][productinfo("prname")],
                    prekes[(int(a) - 1)][productinfo("prprice")],
                    prekes[(int(a) - 1)][productinfo("prdiscount")],
                    prekes[(int(a) - 1)][0],
                    prekes[(int(a) - 1)][productinfo("prphoto1")],
                ]
            ]
        )
    currentpagelist1 = currentpagelist1[
        (int(page) * prperpage - prperpage) : (int(page) * prperpage)
    ]
...
def getproductinfo(productid):
    ...
    for a in prekes:
        if a[0] == productid:
            infodict = {
                "name": a[productinfo("prname")],
                "price": a[productinfo("prprice")],
                "discount": a[productinfo("prdiscount")],
                "description": a[productinfo("prdescription")],
                "material": a[productinfo("prmaterial")],
                "width": a[productinfo("prwidth")],
                "height": a[productinfo("prheight")],
                "thickness": a[productinfo("prthickness")],
                "weight": a[productinfo("prweight")],
                "inputinfo": a[productinfo("prinputinfo")],
                "placeholder": a[productinfo("prplaceholder")],
                "photos": [
                    a[productinfo("prphoto1")],
                    a[productinfo("prphoto2")],
                    a[productinfo("prphoto3")],
                    a[productinfo("prphoto4")],
                    a[productinfo("prphoto5")],
                    a[productinfo("prphoto6")],
                    a[productinfo("prphoto7")],
                    a[productinfo("prphoto8")],
                    a[productinfo("prphoto9")],
                    a[productinfo("prphoto10")],
                ],
                "categories": a[productinfo("prcategory")],
                "type": a[productinfo("prtype")],
                "configuratortype": a[productinfo("configuratortype")],
            }
...
def registerfnc(a, b, g, h):
    ...
    if len(str(g)) > 40:
        ...
        if g in root["users"]:
            ...
            flash(
                "Jūsų pasirinktas pašto adresas jau yra užregistruotas. Bandykite prisijungti arba atnaujinkite slaptažodį."
            )
    if not sha256_crypt.verify(b, a):
...
def handlelikedlistzodb(producttoaddremove):
    ...
    if not session.get("logged_in"):
        if producttoaddremove is not None:
            ...
            listas.extend(
                [
                    [
                        ...
                        edge,
                        filterr,
                        [int(elementh[10]), int(elementh[11])],
                        elementh[3],
                        [
                            textlist[(int(elementh[0]) - 1)],
                            "shop/fonts/" + elementh[7],
                            elementh[8],
                            elementh[9],
                        ],
                    ]
...
def customizedimage():
    ...
    listas = [
        [texture, edge1, edge1, [1, 3], "img", putonimage],
        [texture, edge2, edge2, [1, 3], "img", putonimage],
        [
            texture,
            edge3,
            edge3,
...
def getfieldvalue(
    inputtype, postname
):  ##############################funkcija issaugo paveiksliuka ir grazina jo pavadinima arba texta
    if inputtype == "text":
...
def multipleproductsinputs(parameters, quantity):
    ...
    for num in range(quantity):
        ...
        for a in parameters:
            ...
            if (
                requesttype == "opt1"
            ):  ##################################Pasirinko klientas visiem taikyti ta pati
                if (
                    num != 0
                ):  #################################################Jei nebe pirmas produktas, tai kopijuoti parametra nuo pirmo
                ...
                requesttype == "opt2"
            ):  ##############################Klientas pasirinko taikyti parametra pasirinktinai
...
def setorder(buyerinfo, order_data, orderstatus):
    ...
    file1 = drive.CreateFile(
        {"title": ordersheetname + ".txt", "parents": [{"id": EshopOrderdatatxtfilesID}]}
    ...
    row = [
        ...
        str(date),
        orderstatus,
        buyerinfo["buyer type"],
        buyerinfo["name"],
        "banko acc",
        "suma",
        buyerinfo["email"],
        buyerinfo["phone"],
        buyerinfo["organization name"],
        buyerinfo["organization address"],
        buyerinfo["organization id"],
        buyerinfo["pvm id"],
        buyerinfo["delivery type"],
        buyerinfo["omniva address"],
        buyerinfo["address"],
        buyerinfo["city"],
        buyerinfo["zip code"],
        "",
        ...
        orderlist.append(
            {
                ...
                "discount": int(productinfo["discount"]),
                "productname": productinfo["name"],
                "productid": a["productid"],
                "productnumber": len(a["productlist"]),
            }
        )
    return orderlist
...
def setunpaidorderincsvline(buyerinfo, order_data, fullprice, orderstatus):
    ...
    drive = GoogleDrive(gauth)
    file1 = drive.CreateFile(
        {"title": ordersheetname + ".txt", "parents": [{"id": unpaidorderGoogleDrivefileID}]}
    )  # Create GoogleDriveFile instance with title 'Hello.txt'.
    file1.SetContentString(order_data)  # Set content of the file from given string.
    file1.Upload()
    datatxtfileid = file1["id"]
