...
EshopOrderdatatxtfilesFinishedID = os.environ.get(
    "googlediscfolderid_orders/orderdatatxtfiles/finishedorders"
)
...
def makesvg(orderexelrows):
    ...
    for a in orderexelrows:  #####################dalinam i atskirus uzsakymus
        ...
        for (
            ...
            userdict
        ):  ##############dalinam i atskirus produktus uzsakyme(veliau reiks dalinti i atskirus sio produkto vienetus)
            ...
            with open(
                "shop/templates/svgs/" + singleproduct["productid"] + "mf" + ".svg", "r"
            ) as soup:
            ...
            for everysinglesvggenerator in singleproduct["productlist"]:
                ...
                orderedproductsdata[svgname + ".svg"] = {
                    "product id": singleproduct["productid"],
                    "svgname": svgname + ".svg",
                    "material": prinfo["material"],
                    "thickness": prinfo["thickness"],
                    "width": prinfo["width"],
                    "height": prinfo["height"],
                    "price": prinfo["price"],
                    "weight": prinfo["weight"],
                    "inputdata": everysinglesvggenerator,
                    "status": "SVG only generated",
                    "Who is manufacturing": "Noone yet",
                }
                ...
                if prinfo["type"] == "personalised":
                    for (
                        ...
                        productinfoinputs
                    ):  ######Loopinam per svg pakeiciamus laukelius ir ikeliam userio inputus
                        if feature[2] == "text":
                        ...
                        if feature[2] == "img":
                            if everysinglesvggenerator[feature[1]] in singleproduct["imglist"]:
                                soup.find(id=feature[4])["xlink:href"] = singleproduct["imglist"][
                                    everysinglesvggenerator[feature[1]]
                                ]
                            else:
                ...
                file1 = drive.CreateFile(
                    {"title": svgname + ".svg", "parents": [{"id": SvgFolderID}]}
                )  # Create GoogleDriveFile instance with title 'Hello.txt'.
                file1.SetContentString(soup)  ####soup.prettify()
                file1.Upload()

        OrderStatusManagementfilename = a[22] + "MANAGEMENT.txt"
        orderedproductsdata = json.dumps(
            orderedproductsdata
        )  ######################ikeliam lysna faila kuriame nurodyta uzsakymo sudetis ir busenos
        file1 = drive.CreateFile(
            {"title": OrderStatusManagementfilename, "parents": [{"id": EshopOrderdatatxtfilesID}]}
        )
...
def getorderexelrowsandsvgs(
    orderlist,
):  ######grazina dicta {productid:'svg innerhtml'}  +SUKURIA NAUJUS SVG
...
def getordersvgs(order):  ##### paima orderlista grazina svg failu id
    ...
    SvgProcessedFolderList = drive.ListFile(
        {"q": "'" + SvgProcessedFolderID + "'" + " in parents and trashed=false"}
    ).GetList()
    SvgFolderList = drive.ListFile(
        {"q": "'" + SvgFolderID + "'" + " in parents and trashed=false"}
    ).GetList()
    for a in ordersvgslist:
...
def setSvgsFinished(order, finishedsvgs):
    ...
    svglistinmainfolder = drive.ListFile(
        {"q": "'" + SvgFolderID + "'" + " in parents and trashed=false"}
    ).GetList()
    svglistinInkscapePreparedfolder = drive.ListFile(
        {"q": "'" + SvgInkscapePreparedFolderID + "'" + " in parents and trashed=false"}
    ).GetList()
    svglistinProcessedfolder = drive.ListFile(
        {"q": "'" + SvgProcessedFolderID + "'" + " in parents and trashed=false"}
    ).GetList()
                ...
                if str(orderidlist[e].lower()) in str(a).lower():
                    ...
                    if (
                        ...
                        if paid == int(ordercost[e + 1]):
                            ...
                            errors.append(
                                "Not enough paid: " + str(orderidlist[e]) + "    SMS: " + str(a)
                            )
                        elif paid > int(ordercost[e + 1]):
                    ...
                    #   errors.append('Dublicated payment: '+str(a))
                        ...
                        paystatus[e] != "unpayed"
                    ):  ##########################jei apmoketa tada dadedam nauja suma prie senos
                        errors.append(
                            "dublicated payment for order: "
                            + str(orderidlist[e])
                            + "    SMS: "
                            + str(a)
                        )
...
def checkandaddpaymentsfromsms(
    transactionsmslist,
):  #####NAUDOJA telefono visus sms nuo swedbank. Jei nera sms'o spreadsheete, ideda smsa i payments spreadshyta
...
def manageSMSpaymentdata(
    smslist,
):  #####NAUDOJA telefono visus sms nuo swedbank. Jei nera sms'o spreadsheete, ideda smsa i payments spreadshyta
    try:
        oldsmslist = getFileDotJson(SwedbankPaymentsID)  ################failas kuriame yra visi sms
        unpaidorderslist = getFileDotJson(
            unpaidordersjsonID
        )  ##############failas kuriame visi neapmoketi uzsakymai
        ...
        if (
            len(newsmslist) != oldsmslist
        ):  #######################jei atejo naujas sms irasyti i sms sarasa
            scope = ["https://www.googleapis.com/auth/drive"]
...
def addpaymenterrors(errorlist):
    ...
    for a in errorlist:
        if a not in sheeterrorlist:
            ...
            a in sheeterrorlist
        ):  ###########################jei pasikartojo eroras updatinam jo eilute
            rowindex = sheeterrorlist.index(a)
            currenterrorcounter = int(sheeterrorcounterlist[rowindex])
            sheet.update_cell(
                rowindex + 1, 3, currenterrorcounter + 1
            )  #####skaiciuojam ir updatinam kiek kartu iviko sis error
            sheet.update_cell(
                rowindex + 1, 4, str(datetime.datetime.now())
            )  #####updatinam paskutinio erroro data


def getfilejsonfromdrive(
    fileid,
):  ###################################VEIKIA TIK SU TXT FAILAIS KURIUOSE IRASYTAS JSON
    scope = ["https://www.googleapis.com/auth/drive"]
    credentials = ServiceAccountCredentials.from_json_keyfile_name("shop/creds.json", scope)
    gauth = GoogleAuth()
    gauth.credentials = credentials
...
def getorderdatabyid(num):
    ...
    orderdataids = {
        ...
        "city": 17,
        "zip code": 18,
        "order dict filename": 23,
        "order dict fileID": 24,
    }
    orderdata = {}
...
def generateandsendinvoice(userdata):
    ...
    cartdata = {
        "orderid": userdata["id"],
        "cartprice": userdata["amount(cents)"],
        "deliverytype": userdata["delivery type"],
        "deliveryprice": deliveryprices["ltu"][userdata["delivery type"]],
        "cartlist": cartlist,
    }
    buyerdata = {
        "buyer type": userdata["buyer type"],
        "name": userdata["name"],
        "email": userdata["email"],
        "phone": userdata["phone"],
        "delivery type": userdata["delivery type"],
        "omniva address": userdata["omniva address"],
        "address": userdata["address"],
        "organization name": userdata["organization"],
        "organization id": userdata["organization id"],
        "pvm id": userdata["pvm id"],
        "organization address": userdata["organization address"],
    }
    invoicehtml = render_template(
        "invoice/PreInvoice.html", seller=seller, cartdata=cartdata, buyerdata=buyerdata
    )
    HTML(string=invoicehtml).write_pdf("shop/tmp/preinvoice.pdf")
    h3 = "Sveiki, " + buyerdata["name"] + ","
    emailhtml = render_template(
        "email templates/order.html",
        fullprice=userdata["amount(cents)"],
        h3=h3,
        shopname=shopname,
        seller=seller,
        ordernumber=userdata["id"],
    )
    return emailhtml
